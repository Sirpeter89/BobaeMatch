...
export default function PreferencePage(){
    ...
    const lactoseImages= [["https://media.istockphoto.com/vectors/like-and-dislike-icons-collection-set-thumbs-up-and-thumbs-down-vector-id1090181018?k=6&m=1090181018&s=612x612&w=0&h=-sB6yoP13QTZU3cYT7pddZC3BeXyy0aFlopSh1ZwWMo=", true], ["https://www.seekpng.com/png/detail/490-4906394_thumbs-down-thumbs-down-white-png.png", false]]
    ...
    if(!teaChoice){
        ...
        <div className="TeaContainer">
            {lactoseImages.map((lactose) => (
                <div key={lactose} className="choice" onClick={()=>setLactose(lactose[1])}>
                    <ImageComponent image={lactose[0]} text={`${lactose[1]}`}/>
                </div>
            ))}
        </div>
    } else if (!fruit){
        choiceComponents =
        <div className="TeaContainer">
            {lactoseImages.map((fruit) => (
                <div key={fruit} className="choice" onClick={()=>setFruit(fruit[1])}>
                    <ImageComponent image={fruit[0]} text={`${fruit[1]}`}/>
                </div>
            ))}
        </div>
    } else {
